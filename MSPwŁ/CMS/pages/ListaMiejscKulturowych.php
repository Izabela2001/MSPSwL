...
function getData($conn) {
    $obiekty = [];
    $sql = "SELECT * FROM uf_ObiektyMiejsceKulturowe()";
...
if (isset($_POST['delete'])) {
    ...
    echo "<script>window.location.replace('ListaMiejscKulturowych.php');</script>";
    exit;
}
?>

<main class="ListyObiektow">
    <h1>Miejsca Kulturowe</h1>
    <table>
        <thead>
            <tr>    
                <th></th>
        ...
        <div id="ListaObiektow">
            <form class="FormularzPrzekazywania" action="updateMiejsceKulturowe.php" method="get">
                <label for="IdMiejscaKulturowego">Wybierz numer restauracji:</label>
                <select id="IdMiejscaKulturowego" name="IdMiejscaKulturowego" required>
                    <?php foreach ($obiekty as $obiekt): ?>
                        <option value="<?= htmlspecialchars($obiekt['IdMiejscaKulturowego']) ?>"><?= htmlspecialchars($obiekt['IdMiejscaKulturowego']) ?></option>
                    <?php endforeach; ?>
                </select>
                <button class="btn" type="submit">Aktualizuj Miejsce Kulturowe</button>
            </form>
            <?php foreach ($obiekty as $obiekt): ?>
                <tr>
                    <td>
    ...
    <form action="dodajMiejscaKulturowego.php" >
        <button class="btn" type="submit" >Dodaj nowe Miejsce Kulturowe</button>
    </form>
</main>
