...
<main class="mainM">
    ...
    <ul class="eList">
        <?php
        $sql = "SELECT * FROM VW_MiejsceKulturowe";
        ...
        if (sqlsrv_has_rows($stmt)) {
            while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                ...
                echo "<p><strong>Rodzaj miejsca:</strong> " . (!is_null($row['Typ miejsca']) ? htmlspecialchars($row['Typ miejsca']) : 'Brak danych') . "</p>";
                echo "<p><strong>Osoba zarządająca obiektem </strong></p>";
                echo "<p><strong>Imię:</strong> " . (!is_null($row['Imie']) ? htmlspecialchars($row['Imie']) : 'Brak danych') . "</p>";
