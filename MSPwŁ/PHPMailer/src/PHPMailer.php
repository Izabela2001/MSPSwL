...
class PHPMailer
{
     ...
     * This length does NOT  the line break
     ...
     * The default boundaries  a "=_" sequence which cannot occur in quoted-printable bodies,
    ...
    {
        //It's not possible to use shell commands safely (which s the mail() function) without escapeshellarg,
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] === $disposition_type) {
                ...
                //Only  a filename property if we have one
                if (!empty($name)) {
                    $mime[] = sprintf(
     ...
     * Picks shortest of Q, B, or none. Result s folding if needed.
     ...
     * This can  images, sounds, and just about any other document type.
     ...
     * IPv6 addresses must  [], e.g. `[::1]`, not just `::1`.
     *
    ...
    protected function lang($key)
    {
        ...
        if (array_key_exists($key, $this->language)) {
            if ('smtp_connect_failed' === $key) {
                // a link to troubleshooting docs on SMTP connection failure.
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        foreach ($parsedHeaders as $header) {
            //Is this header one that must be d in the DKIM signature?
        ...
        //The DKIM-Signature header is d in the signature *except for* the value of the `b` tag
     ...
     * Before the send() call, queued addresses (i.e. with IDN) are not yet d.
     *
     * @return array
     */
